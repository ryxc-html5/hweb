  ...
  <div class="container-fluid">
      ...
      <div class="col-md-12">
        <div class="title">
          <div class="form-group">
            ...
            <div class="col-sm-4">
              <input type="text" class="form-control" v-model="popularityData.title" placeholder="不超过10个字符">
            </div>
            <div class="col-sm-6">
              <button type="submit" class="btn btn-default" @click="addGood">新增单品</button>最多添加{{limitedCount}}款
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="row">
      <div class="col-md-12 text-center">
        <div class="row">
          <div class="col-md-6"  v-for="item in popularityData.rows[0].items" track-by="$index">
...
<script>
  ...
  module.exports={
    ...
    methods:{
       ...
       */
      addGood: function () {
        if(this.popularityData.rows[0].items.length>(this.limitedCount-1)){
          swal("最多只能增加"+this.limitedCount+"个商品!", "", "error");
          return;
        }

        this.popularityData.rows[0].items.push({
              "title": "",//主标题
              "subTitle": "",//副标题
              "desc": "",//描述
      ...
      deleteGood: function (data) {
        ...
        swal({
          ...
          if (isConfirm) {
            let items=thiz.popularityData.rows[0].items;
            items.$remove(data);
            swal("成功删除", "", "success");
          } else {
            swal("已取消", "", "error");
      ...
      up: function (data) {
        let index=0;
        let items=this.popularityData.rows[0].items;
