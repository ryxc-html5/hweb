          ...
          <div class="form-group">
            ...
            <div class="col-sm-3">
              <button type="submit"
                      ...
                      @click="addCategory">新增标签</button>最多10个
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="col-md-12 text-center">
          <div class="row">
            <div class="col-md-6"  v-for="item in goodList" track-by="$index">
              <Category-good-image :image-data="item" :del="deleteGood" :up="up" :down="down"></Category-good-image>
            </div>
          </div>
        </div>
      </div>
...
<script>
  ...
  import CategoryGoodImage from '../common/CategoryGoodImage';
  ...
  module.exports = {
    ...
    components: {
      ...
      CategoryGoodImage:CategoryGoodImage
    },
    methods: {
      /**
       * 添加商品品类
      ...
      addCategory: function () {
        if(this.tabData.categorys.length>9){
          swal("最多只能增加10个标签!", "", "error");
          return;
        }else {
          let category={
            name:this.name,//标签名
